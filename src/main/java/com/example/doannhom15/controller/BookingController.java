...
public String confirmBooking(@RequestParam Long showtimeId,
    ...
    // Xác nhận đã thanh toán (user bấm nút)
    ...
    public String confirmPayment(@PathVariable Long bookingId, Model model) {
        try {
            ...
            if (payment.getExpiredAt() != null && payment.getExpiredAt().isBefore(java.time.LocalDateTime.now())) {
                ...
                model.addAttribute("error", "Thời gian thanh toán đã hết hạn");
                return "redirect:/user/payment/" + bookingId;
            }
            ...
            if (b != null) {
                ...
                // Gửi email xác nhận thanh toán + vé qua Gmail/email (kể cả khi có giảm giá)
